...
func (r *LoginRepo) Register(user entity.User) error {
	// 转换数据库模型
...
func (r *LoginRepo) GetUserByEmail(email string) (User entity.User, err error) {
	// 1. 从数据库中查询用户
...
func (r *LoginRepo) GetCaptcha(email string) (captcha entity.Captcha, err error) {
	// 1. 从 redis 读取验证码
	var value string
	value, err = r.rdb.Get(r.ctx, fmt.Sprintf(REDIS_CAPTCHA_KEY, email)).Result()
...
func (r *LoginRepo) DeleteCaptcha(email string) error {
	// 1. 从 redis 中删除验证码
	return r.rdb.Del(r.ctx, fmt.Sprintf(REDIS_CAPTCHA_KEY, email)).Err()
}
